...
MODELS = {
    ...
    "Crowd Monitoring": "yolov8n.pt"  # Using the same model as Object Recognition
}
...
def process_crowd_video(video_path, polygon_points, max_people, model, confidence_threshold):
    if len(polygon_points) < 3:
        st.error("Please select at least 3 points for the polygon")
        return
    ...
    roi_polygon = Polygon(polygon_points)
    ...
    while cap.isOpened() and not stop_button:
        ...
        for result in results:
            ...
            for box in boxes:
                # Check if detection is a person (class 0 in COCO dataset) and meets confidence threshold
        ...
        # Display count
        counter_placeholder.text(f"People count: {people_count}")
        
        # Check if count exceeds maximum
        if people_count > max_people:
            warning_placeholder.error(f"⚠️ WARNING: Number of people ({people_count}) exceeds maximum limit ({max_people})!")
        else:
...
def crowd_monitoring_interface(model, confidence):
    st.title("Crowd Monitoring System")
    
    # Initialize session state for polygon points
    ...
    if video_file is not None:
        ...
        # Display video frame for polygon selection
        ...
        if ret:
            ...
            st.write("Click on the image to create polygon points. Press 'R' to reset points.")
            ...
            # If a point was clicked, add it to the points list
            if value is not None and value != st.session_state.temp_point:
                st.session_state.temp_point = value
                st.session_state.polygon_points.append((value['x'], value['y']))
            
            # Draw the current polygon
            frame_with_polygon = frame_rgb.copy()
            if len(st.session_state.polygon_points) > 0:
                points = np.array(st.session_state.polygon_points, np.int32)
                cv2.polylines(frame_with_polygon, [points.reshape((-1, 1, 2))], 
                            True, (255, 0, 0), 2)
                
                # Draw points
                for point in st.session_state.polygon_points:
                    cv2.circle(frame_with_polygon, point, 5, (0, 255, 0), -1)
            
            st.image(frame_with_polygon)
            
            # Display current points
            st.write("Current polygon points:", st.session_state.polygon_points)
            
            # Get maximum people threshold
            max_people = st.number_input("Maximum number of people allowed", min_value=1, value=5)
            
            # Process button
            if st.button("Start Processing"):
                if len(st.session_state.polygon_points) >= 3:
                    process_crowd_video(tfile.name, st.session_state.polygon_points, max_people, model, confidence)
                else:
                    st.error("Please select at least 3 points to create a polygon")
...
def main():
    ...
    is_crowd_monitoring = selected_model == "Crowd Monitoring"
    ...
    if selected_model == "Vehicle Registration Recognition":
        ...
        return
    elif selected_model == "Crowd Monitoring":
        crowd_monitoring_interface(model, confidence)
        return
    ...
    # Handle Image input
    if source_type == "Image" and not is_product_counter and not is_crowd_monitoring:
    ...
    # Handle Video input
    elif source_type == "Upload Video" and not is_crowd_monitoring:
        play_stored_video(confidence, model, selected_model)
    
    # Handle RTSP input
    elif source_type == "RTSP" and not is_crowd_monitoring:
